...
function calculateOthers(otherGoals, name) {
	...
	const appCIPercentage =
		Math.round(((tasksCompleted.length * 100) / appCI.length) * 100) / 100;

	const result = appCIPercentage ? appCIPercentage : 0;
...
function calculateTotal(results) {
	const percentage =
		results.reduce((acc, goal) => acc + goal.percentage, 0) / results.length;
...
function calculateEnglish(englishRecords) {
	...
	const summary = allTitles.map((title) => {
		...
		const total = baseRecords.find(
			(baseRecord) => baseRecord.title === title
		).record;
	...
	const totalpercentage =
		summary.reduce((acc, percentage) => acc + percentage.percentage, 0) /
		summary.length;
...
function getWeekEnglishNumbers(englishRecords, week) {
	...
	const englishRecordsByWeek = englishRecordsWithWeekNumber.filter(
		(task) => task.week === weekFiltered
	);

	const allTitles = [
		...new Set(englishRecordsWithWeekNumber.map((record) => record.title)),
	];

	const englishRecordsJoined = allTitles.map((title) => {
		const onlyTitle = englishRecordsByWeek.filter(
...
function getTasksNumbers(otherGoals, week) {
	...
	const tasksWithWeekNumber = tasksCompleted.map((task) => {
		return {
			...
			dayOfTheWeek: new Date(task.finishedAt).getDay(),
		};
	});

	const tasksByWeek = tasksWithWeekNumber.filter(
		(task) => task.week === weekFiltered
	);
...
function buildEnglishComparative(englishRecords) {
	const currentWeekEnglishRecords = getWeekEnglishNumbers(
		englishRecords,
		getWeekNumber(new Date())
	);
	const lastWeekEnglishRecords = getWeekEnglishNumbers(
		englishRecords,
	...
	const buildComparative = allTitles.map((title) => {
		const currentWeek = currentWeekEnglishRecords.filter(
			(current) => current.title === title
		)[0].record;
		const lastWeek = lastWeekEnglishRecords.filter(
			(current) => current.title === title
		)[0].record;
...
function getBuildTasksComparative(otherGoals) {
	const currentWeekTasks = getTasksNumbers(
		otherGoals,
		getWeekNumber(new Date())
	);
	const lastWeekTasks = getTasksNumbers(
		otherGoals,
		getWeekNumber(new Date()) - 1
	);
	const allGoalNames = [
		...currentWeekTasks.map((task) => task.goalName),
		...lastWeekTasks.map((task) => task.goalName),
	];
	const uniqueGoalNames = [...new Set(allGoalNames)];

	const buildTasksComparative = uniqueGoalNames.map((goalName) => {
		const currentWeek = currentWeekTasks.filter(
			(task) => task.goalName === goalName
		).length;
		const lastWeek = lastWeekTasks.filter(
			(task) => task.goalName === goalName
		).length;
...
export {
	...
	getTasksNumbers,
	getBuildTasksComparative,
	buildEnglishComparative,
};
